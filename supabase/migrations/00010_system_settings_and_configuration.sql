...
- `id` (uuid, primary key)
- `category` (text) - Setting category (theme, security, notifications, etc.)
- `key` (text) - Setting key (unique within category)
- `value` (jsonb) - Setting value (flexible JSON storage)
- `data_type` (text) - Data type (string, number, boolean, json, color)
...
- `type` (text) - Announcement type (info, warning, success, error)
- `is_active` (boolean) - Whether announcement is currently shown
- `start_date` (timestamptz) - When to start showing
- `end_date` (timestamptz) - When to stop showing
- `target_audience` (text) - Who sees it (all, students, drivers, admins)
...
## 5. Default Settings
Pre-populated with sensible defaults for all categories
*/
...
CREATE POLICY "Active announcements readable by all" ON system_announcements
  FOR SELECT USING (
    is_active = true 
    AND start_date <= now() 
...
CREATE POLICY "All announcements readable by admins" ON system_announcements
  FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  );

CREATE POLICY "Announcements manageable by admins" ON system_announcements
  FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  ) WITH CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
...
CREATE POLICY "Audit log readable by admins" ON settings_audit_log
  FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  );

CREATE POLICY "Audit log insertable by system" ON settings_audit_log
  FOR INSERT WITH CHECK (true);

-- Development: Allow public read
...
-- Function: Reset all settings to defaults
...
BEGIN
  ...
  -- Re-insert defaults (will be done via separate insert)
  RAISE NOTICE 'Settings reset to defaults. Run insert_default_settings() to populate.';
END;
$$;
...
-- Add comments
COMMENT ON TABLE system_settings IS 'Centralized application-wide configuration settings';
...
COMMENT ON FUNCTION get_setting(text, text) IS 'Get a specific setting value';
COMMENT ON FUNCTION update_setting(text, text, jsonb, uuid) IS 'Update a setting with audit trail';
COMMENT ON FUNCTION reset_settings_to_defaults() IS 'Reset all settings to default values';
COMMENT ON FUNCTION get_active_announcements(target_audience) IS 'Get active announcements for specific audience';